...
const userLogin = async function (req, res) {
  try{
    ...
    let passw = await userModel.findOne({ password:password  });
    if (!passw) return res.status(404).send({status: false,message: "Enter a valid Password"});
    ...
    if (!getUsersData) return res.status(401).send({ status: false, message: "This account is allready deleted" });
    
    let token = jwt.sign(
    {userId: getUsersData._id.toString()},"Ram_project")
